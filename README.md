...
- Print / Save as PDF asks the sandboxed iframe for a static HTML snapshot. The host treats that snapshot as untrusted, removes scripts, inline event handlers, dangerous URL protocols, and embedded frames, then loads it into a no-script print iframe before opening the print dialog.
